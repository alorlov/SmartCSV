...
export default class Aucocomplete {
  constructor (model) {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: ({name, displayName, block1, block2, block3}) => {
        ...
        const query = this.model.prepareQuery(this.selectListView.getQuery())
      ...
      didConfirmSelection: (keyBinding) => {
        ...
        this.model.prepareQuery(this.selectListView.refs.queryEditor.getText())
        this.model.confirmSelection(this.selectListView.getSelectedItem().item)
        this.hide()
      },
  ...
  subscribeToCellTextEditor (editorElement) {
    return global.atom.commands.add(editorElement, {
      'core:move-up': (event) => {
  ...
  hookSelectListView () {
    ...
    this.selectListView.getQuery = function() {
      if (this.refs && this.refs.queryEditor) {
        return self.model.prepareQuery(this.refs.queryEditor.getText())
      } else {
        return ''
      }
    }
  ...
  async show () {
    ...
    this.commandsForActiveElement = this.model.getItems()
