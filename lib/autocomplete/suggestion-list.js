...
import SuggestionListView from './suggestion-list-view'
...
export default class SuggestionList {
  ...
  isVisible() {
    ...
    return this.overlayDecoration !== undefined
  }
  ...
  // attachTo (element) {
  //   if (!this.panel) {
  //     this.panel = new SuggestionListView
  ...
  async show (editor) {
    ...
    let bufferPosition = new Point(0,0)//editor.getCursorBufferPosition()
    const marker = this.suggestionMarker = editor.markBufferRange([bufferPosition, bufferPosition])
    this.overlayDecoration = editor.decorateMarker(marker, {type: 'overlay', item: this.panel, position: 'head', class: 'elitecsv-palette'})
