...
export default class ControlPanelView extends View {
  ...
  initialize(runner) {
    ...
    let runView = new ButtonControlView()
    runView.initialize(button)
    this.row[0].appendChild(runView)
    ...
    this.subscriptions.add(this.runner.onDidStart(() => runView.toggle()))
    this.subscriptions.add(this.runner.onDidStop(() => runView.toggle()))
