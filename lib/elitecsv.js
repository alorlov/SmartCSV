  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.workspace.onDidChangeActivePaneItem(item => {
      if(item && typeof item.getPath == 'function') {
        console.log("Workspace -> Change tab ", item.getPath())
      }
    }))
    this.subscriptions.add(atom.workspace.onDidOpen(({uri}) => console.log("Workspace -> Open tab", uri)))
    ...
    this.subscriptions.add(atom.workspace.addOpener((uriToOpen, openerOptions) => {
      ...
      if (choice === 'TextEditor') {
        ...
        this.subscriptions.add(csvEditor.onDidOpen(({editor}) => {
          const matrixEditor = new MatrixTable(editor, addonsMatrixTable)
        }))
        return csvEditor
      }
