...
let url, Range, Table, DisplayTable, TableEditor, Selection, TableElement, TableSelectionElement, CSVConfig, CSVEditor, CSVEditorElement
...
export default class EliteCSV {
  constructor(csvConfig) {

    if (!CSVConfig) { CSVConfig = require('./tablr/csv-config') }
    ...
    this.subscriptions = new CompositeDisposable
    this.reportView = new ReportView(csvConfig.reportViewState)
  ...
  static getView (model) {
    ...
    if (model instanceof TableEditor) {
      ...
      this.subscriptions.add(model.onDidOpen(({editor}) => {
        //console.log('regres-table', editor)
        const matrixEditor = new MatrixTable(editor)
        // matrixEditor.setAutocomplete(libraryAutocomplete)
        //new SyntaxTable(editor)
