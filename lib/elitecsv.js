...
import ReleaseControl from './release-new'
...
var mediator = new Mediator(new Base())
...
class EliteCSV {
  ...
  static initMatrixTableForEditor (editor) {
    const matrixEditor = new MatrixTable(editor, mediator)
  ...
  constructor(state) {
    // const JiraTable = require('./jira-table')
    // const jiraTable = new JiraTable(mediator)
    // jiraTable.applyTable('/home/alexander.orlov/Pictures/EE999.xml')
    // .then(() => { jiraTable.updateStoriesStore() })
    ...
    this.subscriptions = new CompositeDisposable()
    this.reportView = new ReportView(this.csvConfig.reportViewState, mediator)
    mediator.addModule('report-view', this.reportView)

    const runnerModel = new Runner(this.reportView, mediator)
    mediator.addModule('runner', runnerModel)
    ...
    const threads = new Threads(mediator)
    mediator.addModule('threads', threads)
    this.threads = threads //todo - to refactore line

    let CommandsFile = require('./commands-file')
    this.commands = new CommandsFile(mediator, atom.config.get('elitecsv.projectPath') + '/logs/reports')
    mediator.addModule('commands', this.commands)

    let reports = new Reports(mediator)
    mediator.addModule('reports', reports)

    let reportModal = new ReportModal(mediator)
    mediator.addModule('report-modal', threads)

    let Store = require('./store')
    this.db = new Store(mediator)
    mediator.addModule('regression-db', this.db)

    // this.releaseControl = new ReleaseControl()
    ...
    this.subscriptions.add(atom.workspace.addOpener((uriToOpen, openerOptions) => {
      ...
      if (!CSVEditor) { CSVEditor = require('./tablr/csv-editor') }
      let options = {trim: true, relax_column_count: false}//_.clone(@csvConfig.get(uriToOpen, 'options') ? {})
    ...
    this.subscriptions.add(atom.workspace.addOpener((uriToOpen, openerOptions) => {
      ...
      if (protocol !== 'elitecsv:' || host !== 'regression') { return }
      ...
      this.subscriptions.add(csvEditor.onDidOpen(({editor}) => {
        ...
        var rt = new ReleaseTable(editor, release, mediator)
  ...
  setStatusBar (statusBar) {
    ...
    this.releaseControl.attach(statusBar)
  }

  setThreadsView (statusBar) {
    statusBar.addLeftTile({item: this.threads.getView(), priority: 100})
  }
  ...
  checkExtensions(uriToOpen) {
    const extensions = atom.config.get('tablr.supportedCsvExtensions') ||
                       ['csv', 'tsv', 'CSV', 'TSV']
    const eliteProjectPath = atom.config.get('elitecsv.projectPath')

    if (!new RegExp(`\\.(${extensions.join('|')})$`).test(uriToOpen) ||
      !new RegExp(`^${eliteProjectPath}`).test(uriToOpen)) {
     return false
    }
    return true
  }
  ...
  normalizePath () {
    ...
    atom.config.set('elitecsv.projectPath', _.capitalize(path.replace('\\','\/')))
  }
