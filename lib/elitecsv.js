...
export default {
  ...
  activate(state) {
    ...
    // this.subscriptions.add(autocomplete.onDidConfirmSelection(event => {
    //   console.log(event)
    // }))
    //
    // const lib = new Library()
    // this.subscriptions.add(lib.onDidChange(items => {
    //   const autoItems = []
    //   for (var libName in items) {
    //     if (items.hasOwnProperty(libName)) {
    //       const libItems = items[libName]
    //       for (item of libItems) {
    //         autoItems.push({name: item.getName(), displayName: item.getName()})
    //       }
    //     }
    //   }
    //   console.log(autoItems)
    //   autocomplete.setSearchList(autoItems)
    //   autocomplete.show()
    // }))
    // lib.load()
