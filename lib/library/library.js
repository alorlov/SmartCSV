...
export default class Library {
  ...
  parseTable(tableName, table) {
    const colsByName = this.getColumnsByName(table)
    const workColumns = this.getWorkColumns(table)
    ...
    for (var blockName in blockRows) {
      if (blockRows.hasOwnProperty(blockName)) {
        ...
        for (let column of workColumns) {
          ...
          const item = new LibraryCol({lib: tableName, block: blockName, col: colName})
