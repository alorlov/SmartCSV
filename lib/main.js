...
export default {
  ...
  activate(state) {
    ...
    //if (atom.workspace.getActivePaneItem() instanceof CSVEditor) {
      console.log('elitecsv is activating')
  ...
  consumeStatusBar (statusBar) {
    if (this.elitecsv) {
      console.log('activate statusBar')
      // this.elitecsv.setStatusBar(statusBar)
      this.elitecsv.setThreadsView(statusBar)
  ...
  tablrViewProvider (model) {
    ...
    if (model instanceof TableEditor) {
      ...
      subscriptions.add(model.onDidOpen(({editor}) => {
        let editorType = EliteCSV.getTableType(model)//this.eliteConfig.get(model.getPath(), 'editor-type')

        if (editorType == "matrix-table") {
          EliteCSV.initMatrixTableForEditor(editor)
        }
