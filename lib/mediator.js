...
class Mediator {
  ...
  handleRequest (request) {
    switch (request.type) {
      ...
      case "command":
        switch (request.name) {
          case "user-add":
            this.modules['threads'].getThread(request.threadName).addUser(request.data)
            break
        }
        break
      case "command-report":
        switch (request.name) {
          ...
          case "user-stop-all":
            this.modules['threads'].removeThread(request.threadName)
            break

          case "run-stop":
            if(request.result == 1) {
              this.modules['threads'].stopThreadPassed(request.threadName)
            } else {
              this.modules['threads'].stopThreadFailed(request.threadName)
            }
            break

          case "run-start":
              this.modules['threads'].getThread(request.threadName).play()
        }
        break
      default:
  ...
  handleClickThreadReportItem (reportName, reportItemID) {
    ...
    const item = report.getItemByID(reportItemID)
    ...
    let reportView = this.modules['report-view'].getReport(reportName)
    if (reportView == null) {
      this.modules['report-view'].removeActiveReport()
      reportView = this.modules['report-view'].createReport(report)
      this.modules['report-view'].showReport(reportName)
    }

    const top = reportView.getOffsetTopForItem(item)
    this.modules['report-view'].scroller.scrollTop(top)
    setTimeout(() => {
      this.modules['report-view'].focusOnCell(item)
    },500)
  }
  ...
  handleClickReleaseRunCell ({storyID, runID, matrixName}) {
    ...
    .then(data => {
      ...
      // capability to old reports
      if (data.data != undefined) {
        data = data.data
      }

      const reportName = storyID + '-' + runID

      var report = this.modules['reports'].getReport(reportName)
      if (!report) {
        report = this.modules['reports'].createReport(reportName)
      ...
      const failedItems = report.getFailedItems()
  ...
  handleClickReleaseNameCell (path) {
    this.base.openFile(path)
  }
  ...
  getHumanName () {
    return this.base.getCurrentEliteEditor().getHumanName()
  }
