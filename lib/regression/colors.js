  ...
  make (val, row, col) {
    ...
    else if (isWork)
    {
      ...
      //
        ...
        let dotParts = start.split('.', 2)
        let hasDialog = start.match('^[A-Z]+') != null,
            hasDefaultDialog = hasDialog && dotParts.length == 1,
            hasTemporaryDialog = hasDialog && dotParts.length > 1,
            hasUtilitiesAction = !hasDialog && dotParts.lentgh > 1
        ...
        else
        {
          el = ''
          // Temporary dialog
          if (hasTemporaryDialog) el = this.createTemporaryDialog(dotParts[0])

          let action = dotParts.length > 1 ? '.' + dotParts[1] : dotParts[0]
