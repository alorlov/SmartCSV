...
export default class MatrixTable extends Table {
      ...
      for (var libName in items) {
        if (items.hasOwnProperty(libName)) {
          ...
          for (item of libItems) {
            autoItems.push({name: item.getName(), displayName: item.getName(), block1: item.getRequiredVars(), block2: item.getWith(), block3: item.getWithout()})
          }
        }
      }
      console.log(autoItems)
      this.autocomplete.setSearchList(autoItems)
    }))
  }

  subscribeToControls () {
    this.subscriptions.add(this.autocomplete.onDidConfirmSelection(e => {
      console.log(e)
      this.cellEditor.getBuffer().setText('@' + e.type)
      this.tableElement.confirmCellEdit()
      // atom.commands.dispatch('tablr-editor atom-text-editor[mini]', 'core:confirm')
    }))
  ...
  afterSubscribeToCellTextEditor () {
    ...
    this.textEditorSubscriptions.add(this.cellEditor.onDidChange(e => {
      ...
      if (text == '@') {
        //this.autocomplete.update({preserveLastSearch: 'open'})
        this.openLibAutocomplete()
      }
    }))
  }
