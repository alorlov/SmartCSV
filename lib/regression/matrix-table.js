...
import { File, Point, CompositeDisposable, Emitter } from 'atom'
...
export default class MatrixTable extends Table {
  ...
  createRunRequest (from = false) {
    ...
    return new Request({
        ...
        runSelected: true,
        rows: rows,
        stopOnFail: this.mediator.getRunnerOption('run-stop-method')
    })
  }
  ...
  subscribeToControls () {
    ...
    this.subscriptions.add(this.editor.onDidSave(() => {
      const list = this.syntax.getCopypasteList()

      if (list.length > 0) {
        this.cloneCopypasteForList(list)
      }
    }))
  ...
  splitSelectedToRows() {
    ...
    for (let range of ranges) {
      range.end.row--
      range.end.column--
      range.start.column = this.getWorkcolByColumn(range.start.column) || 1
      range.end.column = this.getWorkcolByColumn(range.end.column) || 1
    }
  ...
  splitFromCursorToRows() {
    let editor = this.editor,
        ...
        currentWorkcol = this.getWorkcolByColumn(currentCol) || 1,
        firstWorkcol = 1,
        lastWorkcol = this.getWorkcolByColumn(lastCol) || 1
  ...
  getWorkcolByColumn(column) {
    ...
    } else if (isNaN(firstRow[column] * 1)) {
      return null
    } else {
  ...
  cloneCopypasteForList(list) {
    var csvEditor = this.getCsvEditor()
    var pathSplit = path.parse(csvEditor.getPath())
    const csv = require('csv')
    const options = _.clone(this.options)

    for (let itemName of list) {
      var tableNew = this.syntax.setCopypasteItemForRows(itemName, JSON.parse(JSON.stringify(this.editor.getTable().getRows())))
      csv.stringify(tableNew, options, (err, data) => {
        if (err) { return reject(err) }

        var newPath = pathSplit.dir + path.sep + pathSplit.name + '-' + itemName.substring(1) + pathSplit.ext
        var file = new File(newPath)
        file.writeFile(newPath, data).then(() => {
          //
        })
        .catch(err => {
          throw new Error(err)
