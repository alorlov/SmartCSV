...
export default class ReleaseTable extends Table {
  constructor(editor, release, mediator) {
    super(editor)
    ...
    this.subscriptions.add(this.tableElement.subscribeTo(this.tableElement.body, {
      'click': e => {
        ...
        if (e.ctrlKey && position.column == COL_SCENARIO) {
          ...
          if (!file.existsSync()) {
            atom.confirm({
              ...
              buttons: {
                "Create": () => {
                  ...
                  setTimeout(() => {
                    this.mediator.handleClickReleaseNameCell(cell.matrixPath)
                    console.log("Release -> Create new " + cell.matrixPath)
                  },300)
                },
                "No": () => {}
              }
            ...
            console.log("Release -> Open file " + cell.matrixPath)
          }
        }
      }
    }))
  ...
  createRunRequest (from) {
    ...
    return new Request({
        command: 'start',
        scenario: this.getMatrixName() + '.csv',
        runSelected: false,
        rows: '',
        stopOnFail: 'all',
        saveReportDB: true,
        runName: cell.runName,
  ...
  extendTableElement () {
    ...
    table.classList.add('release-table')
    ...
    table.startCellEdit = function() {
      ...
      if (cell.runID != null) {
        ...
        // })
        self.mediator.handleClickReleaseRunCell(cell)
  ...
  getCellInfo() {
    ...
    let runID = null
    let i = colName.indexOf(', ')
    if (i > -1) {
      runID = colName.substr(i + 2)
    }

    let runName = null
    i = colName.indexOf('~')
    if (i > -1) {
      runName = colName.substr(0, i)
    }
