...
export default class ReportView extends ResizableView {
  ...
  entryClicked(e) {
    ...
    this.tableElement.afterCursorMove()
    return false
  }
  ...
  getTableElement() {
    if (this.tableElement == null) {
      this.tableElement = atom.views.getView(this.csvEditor.editor)
    }
    ...
    this.subscriptions.add(this.csvEditor.onDidChangeModified(() => {
      ...
      for (column in columns) {
        ...
        columns[column].cellRender = function(cell, position) {
          cell.value = cellRender.apply(this, arguments)
  ...
  splitSelectedToRows() {
    let ranges = _.clone(this.csvEditor.editor.getSelectedRanges());
    //this.getTableElement().afterCursorMove()
    //let ranges = [{"end" : {'column' :1, 'row' : 26}, "start" : {'column' :1, 'row' : 25}}]
    //console.log(ranges)
