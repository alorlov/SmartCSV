...
export default class Store {
  ...
  *********/
  ...
  getStories () {
    return this.database.query(`Select * From stories Order By name`)
    .then(rows => {
      const stories = []
      for (row of rows) {
  ...
  getStory (id) {
    return this.database.query(`Select * From stories WHERE story_id = ${id}`)
  }

  loadStories2 () {
    return new Promise(resolve => {
      var query = this.database.query(`Select * From stories Order By after`, (message, rows, fields) => {
        for (row of rows) {
  ...
  addStory () {
    return this.database.query(`Select after From stories Order By story_id Desc Limit 1`)
    .then(after => {
  ...
  addStoryAfter (story) {
    this.database.query(`INSERT stories SET after = ${story.after}`)
  ...
  removeStory (story) {
    this.database.query(`DELETE FROM stories WHERE story_id = ${story.id}`)
    .then(result => {
      console.log(story,result)
      this.database.query(`UPDATE stories SET after = ${story.after} Where after = ${story.id}`)
    })
  }

  setStoryField (story, field, value) {
    this.database.query(`UPDATE stories SET ${field} = '${value.replaceAll('\'', '\\\'')}' Where id = ${story.id}`)
    story[field] = value
  }
  ...
  getStoryScreenshots (story_id, run_id) {
    return this.database.query(`
      Select
        scr.id,
        st.comment
      From run_screenshot as scr
      Left Join steps as st ON st.id = scr.step_id
      Where st.story_id = ${story_id} && st.run_id = ${run_id}
    `)
    .then(rows => {
      return rows
    })
