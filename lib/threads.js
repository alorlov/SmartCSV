...
export default class Threads {
  ...
  stopThreadPassed (name) {
    const thread = this.getThread(name)
    thread.stopPassed()
  }

  stopThreadFailed (name) {
    const thread = this.getThread(name)
    thread.stop()
  }
  ...
  addRequestToThread (request, name) {
    ...
    if (thread == null) {
      thread = this.addThread({
        name,
        scenario: request.getParams().scenario
      })
    }
  ...
  addThread(threadObj) {
    let name = threadObj.name
    if (!name) {
      throw Error('Trying to add thread without "name"')
    }

    let model = new ThreadModel(threadObj)
